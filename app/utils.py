...
def normalize_tle_block(tle_text: str) -> Tuple[str, str, str]:
    ...
    if len(lines) >= 2 and lines[0].startswith("1 ") and lines[1].startswith("2 "):
        name = lines[0] if not lines[0].startswith("1 ") else "UNKNOWN"
...
def fix_tle_line(line: str) -> str:
    # Ensure line is exactly 69 chars
    line = line.rstrip()
    if len(line) < 69:
        line = line.ljust(68) + "0"
    elif len(line) > 69:
        line = line[:68]
    # Fix checksum
    line = line[:68] + tle_checksum(line)
    return line

def validate_tle(tle_text: str) -> Tuple[str, str, str]:
    """Normalize and fix a TLE block to prevent errors."""
...
def generate_safe_tle(original_tle: str, dv_mps: float) -> str:
    name, L1, L2 = validate_tle(original_tle)
